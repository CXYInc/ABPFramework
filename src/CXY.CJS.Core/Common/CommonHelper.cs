...
namespace CXY.CJS.Core.Common
{
    ...
    public static class CommonHelper
    {
        ...
        /// <returns></returns>
        public static CityCode FindByCityName(string CityName)
        {
            List<CityCode> query = ChinaCityQuery;
            foreach (CityCode entity in query)
            {
                if (entity.Name.IndexOf(CityName) != -1)
                {
                    return entity;
                }
            }
        ...
        /// <summary>
        /// 根据省市名称获取省市对象
        /// 优先获取城市对象，如果没有去获取对应的省份对象
        /// </summary>
        /// <param name="CityName">城市名称（格式‘广东省广州’或‘广东广州’或‘广东’或‘广州’或‘广州市’等多种组合）</param>
        /// <returns></returns>
        public static CityCode InByCityName(string CityName)
        {
            List<CityCode> query = ChinaCityQuery;
            if (!string.IsNullOrWhiteSpace(CityName))
            {
                string cityName = CityName.Trim().Replace("省", "").Replace("市", "");
                //先去匹配市
                for (int j = cityName.Length; j >= 2; j--)
                {
                    string findName = cityName.Substring(cityName.Length - j, j);
                    foreach (CityCode city in query)
                    {
                        if (city.Name == findName)
                        {
                            return city;
                        }
                    }
                ...
                for (int j = 1; j <= cityName.Length; j++)
                {
                    string findName = cityName.Substring(0, j);
                    foreach (CityCode city in query)
                    {
                        if (city.Name == findName)
                        {
        ...
        {
            CityCode cityQuery = InByCityName(city);
            string code = "";
            if (cityQuery != null)
            {
                code = cityQuery.ID;
        ...
        #region 单一实例化
        public static List<CityCode> ChinaCityQuery
        {
            get
            {
